...
const menuSections = [
  {
    ...
    items: [
      ...
      },
      {
        ...
        subItems: [
          // Added sub-items for Wallet
          {
            title: "Fund Wallet",
            icon: PlusCircle,
            path: routes.DASHBOARD_FUND_WALLET_PAGE,
          },
        ],
      },
    ],
  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        title: "Bank Transfer",
        icon: ArrowUpRight,
        path: routes.DASHBOARD_BANK_TRANSFERS_PAGE,
      },
    ],
  },
  {
...
export default function Sidebar() {
  ...
  return (
    <aside className="w-64 bg-card h-screen sticky top-0 border-r border-border flex flex-col shadow-sm">
      {/* Header */}
      <div className="px-6 py-6 border-b border-border">
        <img src={LOGO_IMAGE || "/placeholder.svg"} alt="BellCollect Logo" />
      </div>
      ...
      <div className="flex-1 overflow-y-auto scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-transparent px-4 py-6">
        {menuSections.map((section, sectionIndex) => (
          <div key={section.title} className={sectionIndex > 0 ? "mt-8" : ""}>
            <div className="text-xs font-bold text-muted-foreground mb-4 px-3 tracking-wider">{section.title}</div>
                ...
                return (
                  <div key={item.path}>
                    ...
                    >
                      ...
                      {/* Active indicator */}
                      {active && (
                        <div className="absolute right-2 w-2 h-2 bg-primary-foreground rounded-full opacity-80" />
                      )}
                    </Link>
                    {item.subItems &&
                      active && ( // Render sub-items if they exist and parent is active
                          ...
                          {item.subItems.map((subItem) => {
                            ...
                            return (
                              <Link
                                key={subItem.path}
                                to={subItem.path}
                                className={`group flex items-center gap-3 px-3 py-2 rounded-xl text-xs font-medium transition-all duration-200 relative ${
                                  subActive
                              ...
                              >
                                ...
                                {subActive && (
                                  <div className="absolute right-2 w-1.5 h-1.5 bg-primary rounded-full opacity-80" />
                                )}
                              </Link>
                            )
                          })}
      ...
      <div className="p-4 border-t border-border space-y-2">
        <button className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-muted-foreground hover:text-primary hover:bg-primary/10 transition-all duration-200 w-full group">
