...
const Province = {
  getStates: () => data,
  getStateNames: () => Object.values(data).map((item) => item.name),
  getStateByName: (name: keyof DataType) => data[name],
  getStateByValueName: (name: string) => Object.values(data).find((item) => item.name === name),
  getStateByCapital: (capital: string) =>
    Object.values(data).find((v) => v.capital.toLowerCase() === capital.toLowerCase()),
  getLocalGovtCount: (name: keyof DataType) => data[name].locals.length,
  getLocalGovtByName: (name: keyof DataType) => data[name].locals,
  searchByState: (name: string) =>
    Object.values(data).filter((v) => v.name.toLowerCase().indexOf(name.toLowerCase()) > -1),
};
